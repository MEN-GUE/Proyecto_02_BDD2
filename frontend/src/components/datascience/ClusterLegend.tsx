      ...
      {meta.map((m) => (
        ...
        >
          <span
            ...
            style={{ backgroundColor: CLUSTER_COLORS[m.cluster % CLUSTER_COLORS.length] }}
          />
          <div>
            <p className="text-xs font-semibold text-gray-800 dark:text-gray-200">{m.label}</p>
            <p className="text-xs text-gray-400">
              {m.count} clientes · Q{m.avgCredito.toLocaleString(undefined, { maximumFractionDigits: 0 })} cred. prom.
            </p>
          </div>
        </div>
      ))}
