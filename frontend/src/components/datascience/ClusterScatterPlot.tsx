  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
      <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
        Clusters K-Means — Crédito vs Órdenes Totales
      </h3>
      ...
      <ResponsiveContainer width="100%" height={340}>
        <ScatterChart margin={{ top: 10, right: 20, bottom: 20, left: 10 }}>
          ...
          <XAxis dataKey="creditoAprobado" name="Crédito Aprobado" unit=" Q" tick={{ fontSize: 11 }} label={{ value: 'Crédito Aprobado (Q)', position: 'insideBottom', offset: -10, fontSize: 11 }} />
          <YAxis dataKey="totalOrdenes" name="Órdenes" tick={{ fontSize: 11 }} label={{ value: 'Total Órdenes', angle: -90, position: 'insideLeft', fontSize: 11 }} />
          <Tooltip
            ...
            content={({ payload }) => {
              ...
              return (
                <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 text-xs shadow-lg">
                  ...
                  <p>Crédito: Q{c.creditoAprobado.toLocaleString()}</p>
                  <p>Órdenes: {c.totalOrdenes}</p>
                </div>
              )
            }}
          />
          <Legend wrapperStyle={{ fontSize: 11 }} />
          {clusters.map((clusterNum) => {
